...
class CVAE_USM(nn.Module):
    def __init__(self, conv1_in_channels, conv1_out_channels, conv2_out_channels, kernel_size_num, in_features_size,
                 hidden_size, dis_hidden, num_class, num_sub_act, num_temporal_states, reverseLayer_latent_domain_alpha, variance,
                 alpha, beta, gamma, delta, epsilon):
        super(CVAE_USM, self).__init__()

        self.hidden_size = hidden_size
        self.num_class = num_class
        self.num_sub_act = num_sub_act
    ...
    def USM_temporal_extraction(self, predict_all_temporal_state_labels, ST_data, S_data, T_data, device):
        ...
        kmeans = KMeans(n_clusters=self.num_sub_act, random_state=0)
    ...
    def USM_temporal_extraction_GMM_clustering(self, predict_all_temporal_state_labels, S_data, T_data, epoch):
        ...
        gmm = GaussianMixture(n_components=self.num_sub_act, random_state=0)
        gmm.fit(predict_all_temporal_state_numpy_data)
        # Get the cluster labels
        cluster_labels = gmm.predict(predict_all_temporal_state_numpy_data)
        predict_all_temporal_state_labels_list = cluster_labels.tolist()

        #_ = self.draw_tsne(predict_all_temporal_state_numpy_data, predict_all_temporal_state_labels_list, "S_T_all", None)
        ...
        # apply Wasserstein distance to get temporal relation loss
        s_USM = np.concatenate(s_USM)
        gmm_after_USM_S = GaussianMixture(n_components=self.num_temporal_states, random_state=0)
        gmm_after_USM_S.fit(s_USM)

        t_USM = np.concatenate(t_USM)
        gmm_after_USM_T = GaussianMixture(n_components=self.num_temporal_states, random_state=0)
        gmm_after_USM_T.fit(t_USM)

        # Get the cluster centers
        centers_S = gmm_after_USM_S.means_
        centers_T = gmm_after_USM_T.means_
        M = ot.dist(centers_S, centers_T, metric='euclidean')
        a = ot.unif(centers_S.shape[0])
        b = ot.unif(centers_T.shape[0])
        wasserstein_distance = ot.emd2(a, b, M)
    ...
    def update_GMM_clustering(self, ST_data, S_data, T_data, opt, epoch):
        ...
        if S_CLASS_L < 1.0:
            ...
            loss = self.Alpha * RECON_L + self.Beta * KLD_L + self.Gamma * S_CLASS_L
        '''
        opt.zero_grad()
        loss.backward()
        opt.step()
    ...
    def draw_tsne(self, a_USM, a_label, SorT, colors, epoch):
        ...
        all_data_2d = tsne.fit_transform(a_USM)

        # Plotting
        plt.figure(figsize=(10, 6))

        # Creating a color map based on unique labels
        unique_labels = np.unique(a_label)
        if colors == None:
        ...
        for label in unique_labels:
            ...
            indices = np.where(a_label == label)
            data = all_data_2d[indices]

            # Scatter plot for each label
            plt.scatter(data[:, 0], data[:, 1], color=colors(np.where(unique_labels == label)[0][0]),
                        label=f'Label {label}')

        plt.title('t-SNE Visualization of ' + SorT)
        plt.xlabel('t-SNE 1')
        plt.ylabel('t-SNE 2')
        plt.legend()
...
class CVAE_no_USM(nn.Module):
    def __init__(self, conv1_in_channels, conv1_out_channels, conv2_out_channels, kernel_size_num, in_features_size,
                 hidden_size, dis_hidden, num_class, num_sub_act, num_temporal_states, reverseLayer_latent_domain_alpha, variance,
                 alpha, beta, gamma, delta):
        super(CVAE_no_USM, self).__init__()
    ...
    def GMM_clustering(self, predict_all_temporal_state_labels, S_data, T_data):
        # generate USM features for each activity and each user
        # predict_all_temporal_state_labels_list = predict_all_temporal_state_labels.tolist()
        predict_all_temporal_state_numpy_data = predict_all_temporal_state_labels.cpu().detach().numpy()
        ...
        for i in unique_labels:
            S_a_activity_index = [index for index, element in enumerate(S_labels) if element == i]
            S_a_activity_temporal_states = [predict_all_temporal_state_numpy_data[index] for index in
                                            S_a_activity_index]
            s_sequences.append(S_a_activity_temporal_states)

            T_a_activity_index = [index for index, element in enumerate(T_labels) if element == i]
            T_a_activity_temporal_states = [predict_all_temporal_state_numpy_data[index + s_len] for index in
                                            T_a_activity_index]
            t_sequences.append(T_a_activity_temporal_states)

        # apply Wasserstein distance to get temporal relation loss
        s_all = np.concatenate(s_sequences)
        gmm_after_S = GaussianMixture(n_components=self.num_temporal_states, random_state=0)
        gmm_after_S.fit(s_all)

        t_all = np.concatenate(t_sequences)
        gmm_after_T = GaussianMixture(n_components=self.num_temporal_states, random_state=0)
        gmm_after_T.fit(t_all)

        # update
        temporal_state_labels_S = gmm_after_S.predict(s_all)
        temporal_state_labels_T = gmm_after_T.predict(t_all)
        ...
        return temporal_state_labels_S, temporal_state_labels_T
    ...
    def update_GMM_clustering(self, ST_data, S_data, T_data, opt, device):
        ...
        # predict_all_temporal_state_labels = F.softmax(predict_all_temporal_state_labels)
        # predict_all_temporal_state_labels = torch.argmax(predict_all_temporal_state_labels, dim=1)
        temporal_state_labels_S, temporal_state_labels_T = self.GMM_clustering(
        ...
        predict_S_class_labels = self.classify_source(S_z)
        S_CLASS_L = F.cross_entropy(predict_S_class_labels, S_c)
        TEMPORAL_L = torch.tensor([0])

        loss = self.Alpha * RECON_L + self.Beta * KLD_L + self.Gamma * S_CLASS_L + self.Delta * disc_DOMAIN_L
