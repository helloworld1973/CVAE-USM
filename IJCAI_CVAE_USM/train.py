import time
from IJCAI_CVAE_USM.alg.model import CVAE_USM, CVAE_no_USM
...
def GPU_CVAE_USM_train(S_torch_loader, T_torch_loader, ST_torch_loader, global_epoch,
    ...
    best_valid_acc, target_acc, best_epoch = 0, 0, 0
    ...
    for round in range(global_epoch):
        ...
        for step in range(local_epoch):
            ...
            if results['target_cluster_acc'] > target_acc:
                best_source_acc = results['valid_acc']
    ...
    print(f'Best target acc: {target_acc:.4f}')
    print(best_cm)
    log_and_print(f'Best target acc: {target_acc:.4f}', filename=file_name)

    return target_acc, best_cm, best_source_acc, best_epoch


def GPU_CVAE_no_USM_train(S_torch_loader, T_torch_loader, ST_torch_loader, global_epoch,
                          local_epoch, num_classes,
                          num_sub_act, num_temporal_states,
                          conv1_in_channels, conv1_out_channels, conv2_out_channels,
                          kernel_size_num, in_features_size, hidden_size, dis_hidden,
    ...
    algorithm = CVAE_no_USM(conv1_in_channels, conv1_out_channels, conv2_out_channels, kernel_size_num,
                            in_features_size,
                            hidden_size, dis_hidden, num_classes, num_sub_act, num_temporal_states,
                            ReverseLayer_latent_domain_alpha, variance,
