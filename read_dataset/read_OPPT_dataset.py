...
from utils.preprocessing import norm_max
...
class READ_OPPT_DATASET:
    ...
    def data_segment(self, data_x, data_y, activities_ID_list):
        ...
        return required_X_bags, required_Y_bags, required_amount
    ...
    def generate_data_with_required_sensor_channels_and_activities(self, sensor_channels_required, activities_required):
        ...
        #source_data_x = norm_max(np.array(source_data_x))
        #target_data_x = norm_max(np.array(target_data_x))
        ...
        new_target_data_y = []
        for a_activity_ID in activities_ID_list:
            a_activity_index_list = [i for i, j in enumerate(source_data_y) if j == a_activity_ID]

            X_bags_a_activity = source_data_x[a_activity_index_list]
            Y_bags_a_activity = np.array(source_data_y)[a_activity_index_list]
            new_source_data_x.extend(X_bags_a_activity)
            new_source_data_y.extend(Y_bags_a_activity)

            a_activity_index_list = [i for i, j in enumerate(target_data_y) if j == a_activity_ID]

            X_bags_a_activity = target_data_x[a_activity_index_list]
            Y_bags_a_activity = np.array(target_data_y)[a_activity_index_list]
            new_target_data_x.extend(X_bags_a_activity)
            new_target_data_y.extend(Y_bags_a_activity)

        source_data_x = new_source_data_x
        source_data_y = new_source_data_y
        target_data_x = new_target_data_x
        target_data_y = new_target_data_y


        source_required_X_bags, source_required_Y_bags, source_required_amount = self.data_segment(
            source_data_x, source_data_y, activities_ID_list)
        target_required_X_bags, target_required_Y_bags, target_required_amount = self.data_segment(
    ...
    def generate_data_with_required_sensor_channels(self, sensor_channels_required):
        ...
        '''
        source_data_x[:, 6] = source_data_x[:, 6] / 1000 * 9.8
        source_data_x[:, 7] = source_data_x[:, 7] / 1000 * 9.8
        source_data_x[:, 8] = source_data_x[:, 8] / 1000 * 9.8
        target_data_x[:, 6] = target_data_x[:, 6] / 1000 * 9.8
        target_data_x[:, 7] = target_data_x[:, 7] / 1000 * 9.8
        target_data_x[:, 8] = target_data_x[:, 8] / 1000 * 9.8

        source_data_x[:, 9] = source_data_x[:, 9] / 1000
        source_data_x[:, 10] = source_data_x[:, 10] / 1000
        source_data_x[:, 11] = source_data_x[:, 11] / 1000
        target_data_x[:, 9] = target_data_x[:, 9] / 1000
        target_data_x[:, 10] = target_data_x[:, 10] / 1000
        target_data_x[:, 11] = target_data_x[:, 11] / 1000
        '''
